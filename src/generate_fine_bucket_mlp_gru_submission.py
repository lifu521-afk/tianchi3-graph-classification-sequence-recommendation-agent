...
def fine_bucket_weights(lengths: np.ndarray) -> np.ndarray:
    weights = np.empty(len(lengths), dtype=np.float32)
    weights[lengths == 0] = FINE_BUCKET_GRU_WEIGHTS["empty"]
    weights[lengths == 1] = FINE_BUCKET_GRU_WEIGHTS["len1"]
    weights[lengths == 2] = FINE_BUCKET_GRU_WEIGHTS["len2"]
    weights[lengths == 3] = FINE_BUCKET_GRU_WEIGHTS["len3"]
    weights[lengths >= 4] = FINE_BUCKET_GRU_WEIGHTS["len4p"]
    return weights
...
def recommendation_submission() -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    weights = fine_bucket_weights(test_lengths)
    ...
    bucket_counts = {
        ...
        "len2": int((test_lengths == 2).sum()),
        "len3": int((test_lengths == 3).sum()),
        "len4p": int((test_lengths >= 4).sum()),
    }
    log = {
        "task": "recommendation",
        "method": "mlp9_plus_gru3_fine_bucket_weighted_by_history_length",
        "device": str(DEVICE),
        ...
        "mlp_runs": MLP_RUNS,
        "gru_run": GRU_RUN,
        "fine_bucket_gru_weights": FINE_BUCKET_GRU_WEIGHTS,
        "bucket_counts": bucket_counts,
        "seeds": mlp_logs + gru_logs,
        "validation_reference": {
            "coarse_bucket_validation_ndcg": 0.49032857213376563,
            "fine_bucket_validation_ndcg": 0.49048178048924695,
            "gru6_fine_bucket_validation_ndcg": 0.4901595217727845,
            "fine_bucket_weights": FINE_BUCKET_GRU_WEIGHTS,
        },
    }
...
def main() -> None:
    ...
    print("training recommendation fine-bucket MLP+GRU...", flush=True)
